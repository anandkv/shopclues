...
function podSummary(){
	...
	$query="SELECT mf.pickupboy_id ,pb.pickupboy, mf.manifest_id,FROM_UNIXTIME(mf.logout_date, '$timeFomat') logout_dt, FROM_UNIXTIME(mf.login_date, '$timeFomat') login_dt, FROM_UNIXTIME(dispatch_date, '%d-%m-%Y') AS date, SUM( received_qty ) received_qty, SUM( expected_qty ) expected_qty
...
function listMerchants(){
	...
	$query="select merchant_id, FROM_UNIXTIME(mfd.close_date, '$timeFomat' ) as close_dt, company_name, status, company_add, sum(expected_qty) expected_qty, sum(received_qty) received_qty  from
